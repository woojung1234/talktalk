...
const WeatherTalkScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    if (!weather) {
      Alert.alert('오류', '날씨 정보가 없습니다.', [{ text: '확인', onPress: () => navigation.goBack() }]);
      return;
    }
  ...
  const renderWeatherInfo = () => (
    <Animatable.View animation="fadeIn" style={styles.weatherCard}>
      <LinearGradient colors={['#00d2ff', '#3a7bd5']} style={styles.weatherGradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }}>
        <View style={styles.weatherHeader}>
          <Text style={styles.weatherIcon}>{getWeatherIcon(weather)}</Text>
          <View style={styles.weatherInfo}><Text style={styles.weatherTitle}>현재 날씨</Text><Text style={styles.weatherDescription}>{getWeatherDescription(weather)}</Text></View>
        </View>
        {weather.humidity && (
          <View style={styles.weatherDetails}>
            <View style={styles.weatherDetailItem}><Ionicons name="water-outline" size={16} color="rgba(255,255,255,0.8)" /><Text style={styles.weatherDetailText}>습도 {weather.humidity}%</Text></View>
            {weather.precipitation > 0 && (<View style={styles.weatherDetailItem}><Ionicons name="rainy-outline" size={16} color="rgba(255,255,255,0.8)" /><Text style={styles.weatherDetailText}>강수량 {weather.precipitation}mm</Text></View>)}
          </View>
        )}
      </LinearGradient>
    </Animatable.View>
  ...
  const renderConversationTopic = () => {
    if (isLoading) return (<Animatable.View animation="fadeIn" style={styles.loadingContainer}><ActivityIndicator size="large" color="#00d2ff" /><Text style={styles.loadingText}>AI가 날씨에 맞는 대화 주제를 생성 중입니다...</Text></Animatable.View>);
    if (error) return (<Animatable.View animation="fadeIn" style={styles.errorContainer}><Ionicons name="alert-circle-outline" size={48} color="#ef4444" /><Text style={styles.errorText}>{error}</Text><TouchableOpacity style={styles.retryButton} onPress={generateWeatherTopic}><Text style={styles.retryButtonText}>다시 시도</Text></TouchableOpacity></Animatable.View>);
    if (!conversationTopic) return null;
    return (
      <Animatable.View animation="fadeInUp" delay={300} style={styles.topicCard}>
        <View style={styles.topicHeader}><Text style={styles.topicIcon}>{conversationTopic.icon}</Text><Text style={styles.topicCategory}>{conversationTopic.category}</Text></View>
        <View style={styles.topicContent}><Text style={styles.topicLabel}>💬 추천 대화</Text><Text style={styles.topicExample}>"{conversationTopic.example}"</Text><Text style={styles.tipLabel}>💡 사용 팁</Text><Text style={styles.topicTip}>{conversationTopic.tip}</Text></View>
      </Animatable.View>
    );
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <LinearGradient colors={['#00d2ff', '#3a7bd5']} style={styles.header}>
        <View style={styles.headerContent}><TouchableOpacity style={styles.backButton} onPress={() => navigation.goBack()}><Ionicons name="arrow-back" size={24} color="white" /></TouchableOpacity><Text style={styles.headerTitle}>오늘의 날씨 톡</Text><View style={styles.placeholder} /></View>
      </LinearGradient>
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {renderWeatherInfo()}
        <Animatable.View animation="fadeIn" delay={200} style={styles.sectionHeader}><Text style={styles.sectionTitle}>🤖 AI 추천 대화 주제</Text><Text style={styles.sectionDescription}>현재 날씨를 분석하여 가장 적절한 대화 주제를 추천해드립니다</Text></Animatable.View>
        {renderConversationTopic()}
        <Animatable.View animation="fadeInUp" delay={500} style={styles.actionContainer}><TouchableOpacity style={styles.generateButton} onPress={generateWeatherTopic} disabled={isLoading}><LinearGradient colors={isLoading ? ['#94a3b8', '#64748b'] : ['#00d2ff', '#3a7bd5']} style={styles.buttonGradient}><Ionicons name={isLoading ? "hourglass-outline" : "refresh-outline"} size={20} color="white" /><Text style={styles.buttonText}>{isLoading ? '생성 중...' : '새로운 주제 생성'}</Text></LinearGradient></TouchableOpacity></Animatable.View>
        <Animatable.View animation="fadeInUp" delay={600} style={styles.tipsContainer}>
          <Text style={styles.tipsTitle}>📚 날씨 대화 꿀팁</Text>
          <View style={styles.tipItem}><Text style={styles.tipNumber}>1</Text><Text style={styles.tipText}>날씨는 누구나 공감할 수 있는 안전한 대화 주제예요</Text></View>
          <View style={styles.tipItem}><Text style={styles.tipNumber}>2</Text><Text style={styles.tipText}>개인적인 느낌이나 경험을 함께 공유하면 더 자연스러워요</Text></View>
          <View style={styles.tipItem}><Text style={styles.tipNumber}>3</Text><Text style={styles.tipText}>날씨에서 시작해서 계획이나 취미로 대화를 확장해보세요</Text></View>
        </Animatable.View>
      </ScrollView>
    </SafeAreaView>
  );
